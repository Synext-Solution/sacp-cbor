...
use crate::{CborError, ErrorCode, NoopWorkObserver, WorkObserver};
...
/// Encode a value into canonical CBOR bytes while reporting cooperative work checkpoints.
///
/// # Errors
///
/// Returns the first profile, allocation, vector-sink, or work-observer error.
pub fn encode_to_vec_with_observer<T: CborEncode, O: WorkObserver>(
    value: &T,
    observer: O,
) -> Result<Vec<u8>, CborError> {
    let mut encoder = Encoder::with_sink_and_observer(crate::encode::VecSink::new(), observer)?;
    encoder.encode(value).map_err(vector_error)?;
    encoder.finish().map_err(vector_error)
}
...
/// Encode a value into an owned canonical witness while reporting cooperative work checkpoints.
///
/// # Errors
///
/// Returns the first profile, allocation, vector-sink, or work-observer error.
pub fn encode_to_canonical_with_observer<T: CborEncode, O: WorkObserver>(
    value: &T,
    observer: O,
) -> Result<CanonicalCbor, CborError> {
    Ok(CanonicalCbor::new_unchecked(encode_to_vec_with_observer(
        value, observer,
    )?))
}
...
where
    F: FnOnce(
        &mut ValueEncoder<'_, crate::encode::VecSink, NoopWorkObserver>,
